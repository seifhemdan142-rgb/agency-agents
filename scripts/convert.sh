...
#   ollama       — Ollama Modelfiles (~/.ollama/agency-agents/<slug>/Modelfile)
...
#
# Env: OLLAMA_BASE_MODEL overrides the FROM line in generated Modelfiles
#      (default: llama3.1).
...
convert_ollama() {
  local file="$1"
  local name description slug outdir outfile body tq sq
  ...
  # Stage one dir per agent (install.sh copies into ~/.ollama/agency-agents/<slug>/).
  outdir="$OUT_DIR/ollama/$slug"
  outfile="$outdir/Modelfile"
  mkdir -p "$outdir"

  # Ollama Modelfile: a FROM base model plus the persona as the SYSTEM prompt.
  # """ is the ONLY multiline fence the Modelfile grammar accepts (''' is not
  # one — Ollama ends the value at the newline and then parses the persona as
  # commands), and the grammar has no escape for it. Personas that embed """
  # do so for Python docstrings, where ''' is equivalent, so rewrite those
  # inner fences instead of trying to escape them.
  tq='"""'
  sq="'''"
  body="${body//$tq/$sq}"

  cat > "$outfile" <<HEREDOC
# ${name} — generated by The Agency (scripts/convert.sh); edits are overwritten.
# ${description}
FROM ${OLLAMA_BASE_MODEL:-llama3.1}
...
main() {
  ...
  local valid_tools=("antigravity" "gemini-cli" "opencode" "cursor" "aider" "windsurf" "openclaw" "qwen" "zcode" "kimi" "codex" "osaurus" "ollama" "hermes" "vibe" "all")
  local valid=false
  ...
  local tools_to_run=()
  if [[ "$tool" == "all" ]]; then
    tools_to_run=("antigravity" "gemini-cli" "opencode" "cursor" "aider" "windsurf" "openclaw" "qwen" "zcode" "kimi" "codex" "osaurus" "ollama" "hermes" "vibe")
  else
    tools_to_run=("$tool")
  fi
  ...
  if $use_parallel && [[ "$tool" == "all" ]]; then
    ...
    local parallel_tools=(antigravity gemini-cli opencode cursor openclaw qwen zcode kimi codex osaurus ollama hermes vibe)
