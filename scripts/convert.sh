...
convert_ollama() {
  local file="$1"
  local name description slug outdir outfile body delim
  ...
  # Multiline values are fenced with triple quotes and the grammar has no escape
  # for the fence, so pick one the persona does not contain — many personas
  # embed Python snippets that already use the double-quote fence.
  delim="'''"
  if [[ "$body" == *"'''"* ]]; then
    if [[ "$body" == *'"""'* ]]; then
      error "ollama: $name uses both triple-quote fences in its body — no usable SYSTEM fence"
      return 1
    fi
    delim='"""'
  fi
